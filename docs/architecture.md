...
`TaskGroup`, allocation-free `OneShotEvent`, `RunLoop`, and its copyable `Scheduler` handle. The
...
- the root module `mcpplibs.cmp` with Task, RunLoop, `when_all`, TaskGroup, and event partitions;
...
target. `mcpp test` discovers five test files and links a gtest entry point for each. The tests
...
A final coroutine catches `OperationCancelled` from a pre-cancelled timed wait and prints
`Coroutine cancelled`.
...
Task, RunLoop, `when_all`, TaskGroup, and OneShotEvent occupy separate module partitions because
...
The expected result is a successful library build, 69 passing tests across five binaries, and an
example that prints `Coroutine result: 42`, `Concurrent result: 42`, `Task group result: 42`,
`Event signalled`, then `Coroutine cancelled` and exits with status 0. Tests perform one million
immediate Task completions, 100,000 immediate two-Task joins, 50,000 immediate two-Task vector
joins, 50,000 eager TaskGroup completions, 50,000 event waiters, 100,000 explicit schedules,
